...
const AdminLayout: React.FC<Props> = ({
  ...
  return (
    <div className="min-h-screen mx-auto max-w-7xl text-[#0b1410] pb-20 font-sans relative bg-[#f7f9f8]">
      <div
      ...
      <div className="sticky top-[18px] z-50 flex justify-center px-4 sm:px-7 mb-[38px]">
        ...
        >
          <div className="flex items-center gap-2.5 pr-2 sm:pr-3.5 mr-1 sm:mr-1.5 border-r border-white/10 shrink-0">
          ...
          <div className="flex items-center gap-0.5 flex-1 min-w-0">
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-[7px] py-2 sm:py-[9px] px-3 sm:px-4 rounded-full text-[10px] sm:text-[12px] font-semibold whitespace-nowrap transition-all duration-250 ${isActive
                  ? 'text-[#06150c] bg-[#22c55e]'
                  : 'text-[#9aa79f] hover:text-white hover:bg-white/5 bg-transparent'
                }`
              }
            >
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-7 relative z-10">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between mb-[30px] gap-4">
          <div>
            ...
            <h1 className="text-2xl flex flex-wrap gap-x-2 sm:text-[32px] font-bold tracking-[-0.02em] text-[#0b1410]" style={{ fontFamily: 'Outfit, sans-serif' }}>
              Welcome Admin, <span className="text-[#0f8a44]">{user.name}</span>
            </h1>
            <p className="mt-1.5 text-[#5d6b62] text-sm sm:text-[14.5px]">Approve users and monitor your local weather</p>
          </div>
        </div>
